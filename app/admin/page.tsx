...
export default function PlatformAdminPage() {
  ...
  function selectLogo(event: ChangeEvent<HTMLInputElement>) {
    ...
    void uploadLogo(file).then((url) => { setLogoUrl(url); setMessage("Logo armazenada no Supabase Storage."); }).catch((uploadError) => { setLogoUrl(""); setError(uploadError instanceof Error ? uploadError.message : "Falha no envio da logo."); }).finally(() => setLogoUploading(false));
  }
  ...
  function selectEditLogo(event: ChangeEvent<HTMLInputElement>) {
    ...
    void uploadLogo(file, selectedCompany?.id).then((url) => { setEditLogoUrl(url); setMessage("Nova logo armazenada no Supabase Storage."); }).catch((uploadError) => setError(uploadError instanceof Error ? uploadError.message : "Falha no envio da logo.")).finally(() => setEditLogoUploading(false));
  }

  async function createCompany(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(""); setMessage("");
    if (logoUploading || logoUrl.startsWith("data:")) { setError("Aguarde a conclusão do envio da logo antes de salvar a empresa."); return; }
  ...
  async function updateCompany(event: FormEvent<HTMLFormElement>) {
    ...
    setError(""); setMessage("");
    if (editLogoUploading) { setError("Aguarde a conclusão do envio da logo antes de salvar as alterações."); return; }
  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <div className="mx-auto max-w-7xl px-5 py-6 md:px-8">
        ...
        <div>
          <form onSubmit={createCompany} className={`${tab === "new" ? "" : "hidden"} rounded-3xl border border-slate-200 bg-white p-6 shadow-sm`}>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Logo da empresa">
                  <span className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs font-normal text-slate-500">{logoUploading ? "Enviando para o Supabase Storage..." : logoFileName || "Nenhum arquivo selecionado"}</span>
                  </span>
            ...
            <h3 className="mt-7 font-semibold">Módulos liberados</h3><p className="mt-1 text-sm text-slate-500">Módulos são recursos contratados pela empresa e não dependem do perfil de cada usuário.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 md:grid-cols-3">{MODULES.map(([key, label]) => <label key={key} className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm"><input type="checkbox" name={`module-${key}`} defaultChecked />{label}</label>)}</div>
            <button disabled={logoUploading} className="mt-6 rounded-xl bg-blue-700 px-5 py-3 text-sm font-semibold text-white disabled:cursor-wait disabled:opacity-60">{logoUploading ? "Aguardando envio da logo..." : "Salvar empresa licenciada"}</button>
          </form>
          ...
          <div className="space-y-6">
            <form onSubmit={createUser} className={`${tab === "users" ? "" : "hidden"} rounded-3xl border border-slate-200 bg-white p-6 shadow-sm`}><h2 className="text-xl font-semibold">Usuários de {selectedCompany?.name || "empresa"}</h2><p className="mt-1 text-sm text-slate-500">Cadastre, corrija ou inative acessos sem apagar o histórico.</p><div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              <Field label="Perfil"><select name="role" className="workspace-input mt-1"><option value="owner">Responsável, controle principal da empresa</option><option value="admin">Administrador, gerencia usuários e configurações</option><option value="manager">Gestor, atuação operacional</option><option value="member">Usuário, atuação operacional básica</option></select><small className="mt-1 block text-slate-500">O perfil controla permissões. Os módulos disponíveis são definidos no cadastro da empresa.</small></Field>
      ...
      {editOpen && selectedCompany ? <EditCompanyModal company={selectedCompany} onClose={() => setEditOpen(false)} onSubmit={updateCompany} logoUrl={editLogoUrl} logoUploading={editLogoUploading} brandName={editBrandName} primaryColor={editPrimaryColor} secondaryColor={editSecondaryColor} onLogoChange={selectEditLogo} onBrandNameChange={setEditBrandName} onPrimaryChange={setEditPrimaryColor} onSecondaryChange={setEditSecondaryColor} /> : null}
    </main>
  );
}
...
function roleDescription(role: string) { return ({ owner: "Controle principal, usuários, configurações, Daily e operação.", admin: "Gerencia usuários, configurações, Daily e operação.", manager: "Conduz a Daily e gerencia a operação, sem configurações.", member: "Sem Daily ou configurações; consulta projetos e atua nas atividades vinculadas." } as Record<string, string>)[role] || ""; }

function RoleAccessMatrix() {
  const rows = [
    ["Configurações e usuários", "Sim", "Sim", "Não", "Não"],
    ["Conduzir Daily", "Sim", "Sim", "Sim", "Não"],
    ["Criar e editar clientes/projetos", "Sim", "Sim", "Sim", "Não"],
    ["Consultar projetos vinculados", "Sim", "Sim", "Sim", "Sim"],
    ["Atuar em atividades vinculadas", "Sim", "Sim", "Sim", "Sim"],
  ];
  return <section className="rounded-3xl border border-blue-100 bg-blue-50 p-6"><h2 className="text-lg font-semibold">Matriz de acesso por perfil</h2><p className="mt-1 text-sm text-slate-600">Os módulos contratados controlam as áreas disponíveis. O perfil controla o que cada usuário pode fazer nessas áreas.</p><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead><tr className="border-b border-blue-200"><th className="p-3">Acesso</th><th className="p-3">Responsável</th><th className="p-3">Administrador</th><th className="p-3">Gestor</th><th className="p-3">Usuário</th></tr></thead><tbody>{rows.map((row) => <tr key={row[0]} className="border-b border-blue-100 last:border-0">{row.map((cell, index) => <td key={cell + index} className={`p-3 ${index === 0 ? "font-medium" : cell === "Sim" ? "font-semibold text-emerald-700" : "text-slate-500"}`}>{cell}</td>)}</tr>)}</tbody></table></div></section>;
}
