...
export function KairosPanel() {
  const { isOpen, seedMessage, seedProjectId, closePanel, chatEnabled, voiceEnabled } = useKairosPanel();
    ...
    <div className="kairos-panel-overlay" role="presentation">
      ...
      <aside role="dialog" aria-modal="true" aria-label="Kairos" className="kairos-panel-shell">
        ...
        <footer className="kairos-panel-footer">
          {voiceEnabled ? <Link href="/voice" onClick={closePanel}>Abrir Voice Room</Link> : null}
          {chatEnabled ? <Link href="/chat" onClick={closePanel}>Abrir histórico completo</Link> : null}
        </footer>
      </aside>
